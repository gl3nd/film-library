...
function favoriteFilms(f) { return f.filter(f=>f.favorite===true ||f.favorite===1)};

function bestRatedFilms(f){ return f.filter(f=>f.rating===5)};

function unseenFilms(f) {return f.filter(f=> !f.watchDate)};

function seenLastMonth(f) { return f.filter(f => dayjs().subtract(1, 'month').isAfter(f.watchDate) === false && f.watchDate)};



//export default FILMS;
export {seenLastMonth,bestRatedFilms,favoriteFilms,unseenFilms};